...
import 'add_scans.dart';

class AllergiesAiAnalysisLayoutPage extends StatefulWidget {
  String FromDate, ToDate, AnalysisModule;
  final AllergiesAnalysisStorage storage;
  final AllergiesPdfReport PrintPdf;
  final AllergiesRecommendations RecommendationsStorage;

  AllergiesAiAnalysisLayoutPage({ required this.FromDate, required this.ToDate, required this.AnalysisModule, Key? key, required this.storage, required this.PrintPdf, required this.RecommendationsStorage}) : super(key: key);

  @override
  _AllergiesAiAnalysisLayoutPageState createState() => _AllergiesAiAnalysisLayoutPageState(this.FromDate, this.ToDate, this.AnalysisModule);
}

class _AllergiesAiAnalysisLayoutPageState extends State<AllergiesAiAnalysisLayoutPage>  {

  var FromDate;
  var ToDate;
  var AnalysisModule;
  _AllergiesAiAnalysisLayoutPageState(this.FromDate, this.ToDate, this.AnalysisModule);
  ...
  void initState() {
    ...
    widget.storage._readData(ModuleFileName,ModuleDirName).then((String value) async{
      if(value != "No Data")
      {
        ...
        LineSplitter newls = const LineSplitter();
        ...
        if(newlines.isNotEmpty)
        {
          ...
          AllergiesData = List.generate(row, (s) => List.filled(col, "", growable: false), growable: false);

          for (var i = 0; i < newlines.length; i++) {
            List<String> result =  newlines[i].split('#');
            for (var j = 0; j < ListSize; j++) {
              AllergiesData[i][j] = result[j];
            }
          }
        }


        RecommendationsData = List.generate(AllergiesData.length, (s) => List.filled(9, "", growable: false), growable: false);

        for (var i = 0; i < AllergiesData.length; i++) {
          widget.RecommendationsStorage._SearchRecommendationsBank(int.parse(AllergiesData[i][2].toString())).then((String Recommendation) async{
            if(Recommendation != "") {
              AllergiesData[i][5] = Recommendation;
            }
            else
            {
              AllergiesData[i][5] = "No Recommendations for" + AllergiesData[i][1] + "found in our database!!!";
              Fluttertoast.showToast(
                  msg: "No Recommendations for" + AllergiesData[i][1] + "found in our database!!!",
            ...
            Navigator.pushReplacement(context,
                MaterialPageRoute(builder:
                    (context) =>
                        Directionality( // use this
                            textDirection: LanguageData == 1 ?ui.TextDirection.rtl :ui.TextDirection.ltr ,
                            child: AddPrescriptions(storage: AddPrescriptionsStorage(),)
                )),
            );
            break;
          case "Lab Tests":
            Navigator.pushReplacement(context,
                MaterialPageRoute(builder:
                    (context) =>
                        Directionality( // use this
                            textDirection: LanguageData == 1 ?ui.TextDirection.rtl :ui.TextDirection.ltr ,
                            child: AddLabTests(storage: AddLabTestsStorage(),)
                )),
            );
            break;
          case "Vaccinations":
            Navigator.pushReplacement(context,
                MaterialPageRoute(builder:
                    (context) =>
                        Directionality( // use this
                            textDirection: LanguageData == 1 ?ui.TextDirection.rtl :ui.TextDirection.ltr ,
                            child: AddVaccinations(storage: AddVaccinationsStorage(),)
                )),
            );
            break;
          case "Symptoms":
            ...
            Navigator.pushReplacement(context,
                MaterialPageRoute(builder:
                    (context) =>
                        Directionality( // use this
                            textDirection: LanguageData == 1 ?ui.TextDirection.rtl :ui.TextDirection.ltr ,
                            child: AddScans(storage: AddScansStorage(),)
                )),
            );
            break;
          case "Allergies":
                              ...
                              for (var rowData in Data)
                                rowData[0].toString() != "" && rowData[0] != null
                                    ...
                                    child:  ExpansionTile(
                                      ...
                                      subtitle: Text("Allergy:        " +rowData[1].toString()+ " / " +rowData[3].toString()+'\n'
                                          +"Description:        " + rowData[4].toString(),textAlign: TextAlign.left,
                                        style: const TextStyle(height: 1.5),
                                      ),
                                      children: <Widget>[
                                        ListTile(title: Text('Recommendations for ' + rowData[1].toString()+ ' Allergy',textAlign: TextAlign.center,
                                          style: const TextStyle(
                                            decoration: TextDecoration.underline,
                                            color: Colors.teal,
                                            fontSize: 16,
                                        ...
                                        ListTile(title: rowData[5].toString() != ""
                                            ?Text(rowData[5].toString(),textAlign: TextAlign.start,
                                          style: const TextStyle(
                                            color: Color.fromRGBO(32, 116, 150, 1.0),
                                            fontSize: 14,),)
                                            :Text('No Recommendations for ' + rowData[1].toString() + " Allergy found in our database.",textAlign: TextAlign.center,
                                          style: const TextStyle(
                                            color: Colors.teal,
                                            fontSize: 14,
                                            fontWeight: FontWeight.w900,),)),
...
class AllergiesAnalysisStorage
{
...
class AllergiesPdfReport
{
  ...
  Future<void> SaveToPdf(String ModuleName, List Data,List Recomendations,String FromDate,String ToDate,String AnalysisModule,String ModuleImage) async {
    ...
    final ByteData ModuleIconbytes = await rootBundle.load('assets/img/allergies32.png');
          ...
          build: (pw.Context context) => pw.Padding(
              ...
              child: pw.Column(
                  ...
                  children: [
                    ...
                    pw.Row(
                        ...
                        children: [
                          ...
                          pw.Column(
                              ...
                              mainAxisAlignment: pw.MainAxisAlignment.center,
                              children: [
                                pw.Text("Allergy : "+Data[i][1]+" / "+Data[i][3],
                                    style: const pw.TextStyle(fontSize: 14)),
                              ]
                          ),
                        ]
                    ...
                    pw.SizedBox(height: 10),
                    pw.Text("Recommendations for "+ Data[i][1] + " Allergy",
                        style: const pw.TextStyle(fontSize: 14,color: PdfColors.teal,)),
                    ...
                    // pw.SizedBox(height: 5),
                    // pw.Text("Never Take Any Medicines Without Consulting Your Doctor",
                    //     style: const pw.TextStyle(fontSize: 14,color: PdfColors.teal,)),
                  ]
              )
          ),
...
class AllergiesRecommendations
{
  Future<String> _readRecommendationsBank() async {
    return await rootBundle.loadString('assets/files/AllergiesAiBank.txt');
  }
