...
import 'Dependent/DependentPages/DependentHomePage.dart';


class DependentsPage extends StatefulWidget {
  final DependentsStorage storage;

  DependentsPage({Key? key, required this.storage}) : super(key: key);

  @override
  _DependentsPageState createState() => _DependentsPageState();
...
class _DependentsPageState extends State<DependentsPage>  {
  ...
  var NoAddedTitle = ["No added Family Members, Please Add new","لا توجد أفراد أسرة مضافة ، الرجاء إضافة جديد",""];
  ...
  void initState() {
    ...
    widget.storage._readData("icare_Dependents").then((String value) async{
      if(value != "No Data" && value != "")
      {
      ...
      else
      {
        Fluttertoast.showToast(
            msg: "No added family members, Please Add now.",
