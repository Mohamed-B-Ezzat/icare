...
class DependentSymptomsPage extends StatefulWidget {
  final DependentSymptomsStorage storage;
  final String DirName;
  final String DataTitle;
  final String DataImage;
  DependentSymptomsPage({Key? key,required this.DataTitle, required this.DataImage,required this.DirName, required this.storage}) : super(key: key);

  @override
  _DependentSymptomsPageState createState() => _DependentSymptomsPageState(this.DataTitle, this.DataImage,this.DirName);

}

class _DependentSymptomsPageState extends State<DependentSymptomsPage>  {
  var DirName;
  var DataTitle;
  var DataImage;

  _DependentSymptomsPageState(this.DataTitle, this.DataImage,this.DirName);
  ...
  void initState() {
    ...
    widget.storage._readData(DirName.toString(),"icare_symptoms").then((String value) async{
      if(value != "No Data" && value != "")
        {
      ...
      else
        {
          Fluttertoast.showToast(
              msg: "No added DependentSymptoms, Please Add now.",
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      //extendBodyBehindAppBar: true,
        ...
        appBar: AppBar(
          title: const Text('Family Member Symptoms',
            style: TextStyle(
              color: Colors.white,),
          ),
          centerTitle: true,
                        ...
                        child: Data != null && ImageList != null
                          ...
                          children: [
                            ...
                            const Card(
                              ...
                              child: ListTile(
                                title: Text("No added DependentSymptoms, Please Add new",
...
class DependentSymptomsStorage
{
...
// Find the Documents path
  ...
  Future<String> createFolder(String DirName) async {
    var dir = await getApplicationDocumentsDirectory();
    final path = Directory('${dir.path}/ICare/'+DirName+'/Symptoms');
    var status = await Permission.storage.status;
    if (!status.isGranted) {
      await Permission.storage.request();
    }
      ...
      child: ListTile(
        ...
        title: GestureDetector(
            ...
            onTap: (){
              ...
              //     MaterialPageRoute(builder:
              //         (context) =>
              //             DependentAddSymptoms(DirName: DirName.toString(),storage: DependentAddSymptomsStorage(),)
              //     )
              // );
            }
        ),
              ...
              GestureDetector(
                  ...
                  onTap: (){
                    Navigator.pushReplacement(context,
                        ...
                        Directionality( // use this
                            textDirection: LanguageData == 1 ?TextDirection.rtl :TextDirection.ltr ,
                            child: DependentDeleteItemListPage(DirName: 'DependentSymptoms', FileName: 'icare_DependentSymptoms', DataIndex: DataIndex, DataTitle: DataSubTitle, DataLines: DataLines, DataImage: "DependentSymptoms")
                        )
                        )
                    );
                  }
