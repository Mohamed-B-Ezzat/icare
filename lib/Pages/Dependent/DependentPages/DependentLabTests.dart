...
import 'DependentEditViewLabTests.dart';
import 'Dependentadd_labtests.dart';


class DependentLabTestsPage extends StatefulWidget {
  final DependentLabTestsStorage storage;
  final String DirName;
  final String DataTitle;
  final String DataImage;
  DependentLabTestsPage({Key? key,required this.DataTitle, required this.DataImage,required this.DirName, required this.storage}) : super(key: key);

  @override
  _DependentLabTestsPageState createState() => _DependentLabTestsPageState(this.DataTitle, this.DataImage,this.DirName);
}
class _DependentLabTestsPageState extends State<DependentLabTestsPage>  {

  var DirName;
  var DataTitle;
  var DataImage;
  _DependentLabTestsPageState(this.DataTitle, this.DataImage,this.DirName);
  ...
  void initState() {
    ...
    widget.storage._readData(DirName.toString(),"icare_labtests").then((String value) async{
      if(value != "No Data" && value != "")
      {
      ...
      else
      {
        Fluttertoast.showToast(
            msg: "No added DependentLabTests, Please Add now.",
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      //extendBodyBehindAppBar: true,
        ...
        appBar: AppBar(
          title: const Text('Family Member Lab Tests',
            style: TextStyle(
              color: Colors.white,),
          ),
          centerTitle: true,
                  ...
                  child: Data != null && ImageList != null
                    ...
                    children: [
                      ...
                      const Card(
                        ...
                        child: ListTile(
                          title: Text("No added DependentLabTests, Please Add new",
...
class DependentLabTestsStorage
{
...
// Find the Documents path
  ...
  Future<String> createFolder(String DirName) async {
    var dir = await getApplicationDocumentsDirectory();
    final path = Directory('${dir.path}/ICare/'+DirName+'/LabTests');
    var status = await Permission.storage.status;
    if (!status.isGranted) {
      await Permission.storage.request();
    }
...
class DataListViewWidget extends StatelessWidget {
  ...
  const DataListViewWidget({required Key? key,required this.DirName,required this.LanguageData, required this.DataTitle, required this.DataSubTitle,required this.DataIndex, required this.DataLines, required this.imageURL}) : super(key: key);
        ...
        title: GestureDetector(
            ...
            onTap: (){
              Navigator.pushReplacement(context,
                  ...
                  Directionality( // use this
                      textDirection: LanguageData == 1 ?TextDirection.rtl :TextDirection.ltr ,
                      child: EditViewLabTestsPage(DirName: DirName.toString(),type: false, Data: DataTitle+"\n"+DataSubTitle+"\n")
                  )
                  )
              );
            }
              ...
              GestureDetector(
                  ...
                  onTap: (){
                    Navigator.pushReplacement(context,
                        ...
                        Directionality( // use this
                            textDirection: LanguageData == 1 ?TextDirection.rtl :TextDirection.ltr ,
                            child:  DependentDeleteItemListPage(DirName: DirName.toString(), FileName: 'icare_labtests', DataIndex: DataIndex, DataTitle: DataSubTitle, DataLines: DataLines, DataImage: "DependentLabTests")
                        )
                        )
                    );
                  }
