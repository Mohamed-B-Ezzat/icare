...
class VaccinationsPage extends StatefulWidget {
  final VaccinationsStorage storage;

  VaccinationsPage({Key? key, required this.storage}) : super(key: key);
...
class _VaccinationsPageState extends State<VaccinationsPage>  {
  var LanguageData =0;
  var Title = ["Vaccinations","التطعيمات",""];
  var AddNewTitle = [" + Add new"," + اضافة جديد",""];
  var NoAddedTitle = ["No added Vaccinations, Please Add new","لا توجد تطعيمات مضافة ، الرجاء إضافة جديد",""];
  ...
  void initState() {
    ...
    widget.storage._readData("icare_vaccinations").then((String value) async{
      if(value != "No Data" && value != "")
      {
  ...
  void _handleURLButtonPress(BuildContext context, var type) {
    Navigator.push(context,
        ...
        textDirection: LanguageData == 1 ?TextDirection.rtl :TextDirection.ltr ,
        child: ImageFromGalleryEx("/Vaccinations", type,storage: ImageStorage(),))),);
  }
...
// Find the Documents path
  ...
  Future<String> createFolder() async {
    var dir = await getApplicationDocumentsDirectory();
    final path = Directory('${dir.path}/ICare/Vaccinations');
    var status = await Permission.storage.status;
    if (!status.isGranted) {
      await Permission.storage.request();
    }
              ...
              GestureDetector(
                  ...
                  onTap: (){
                    Navigator.pushReplacement(context,
                        ...
                        Directionality( // use this
                            textDirection: LanguageData == 1 ?TextDirection.rtl :TextDirection.ltr ,
                            child: DeleteItemListPage(DirName: 'Vaccinations', FileName: 'icare_Vaccinations', DataIndex: DataIndex, DataTitle: DataSubTitle, DataLines: DataLines, DataImage: "vaccinations")
                        )),
                    );
                  }
              ),
